...
try:
    with open('china_telecom_exchange.log') as fr:
        dhjl = json.load(fr)
except:
    dhjl = {}
if yf not in dhjl:
            ...
            if r["biz"] != {} and r["biz"]["resultCode"] in errcode:
                ...
                if r["biz"]["resultCode"] in ["0","412"]:
                    ...
                    if phone not in dhjl[yf][title]:
                        dhjl[yf][title] += "#"+phone
                        with open('china_telecom_exchange.log', 'w') as f:
...
def first_request(res=''):
    ...
    with open("china_telecom_encryption.js") as f:
        js_code_ym = f.read()
...
def main():
    ...
    if '$_ts=window' in r.text:
        ...
        print("瑞数加密已关闭")
        rs = 0
    acct_val = os.environ.get('chinaTelecomAccount')
    if not acct_val:
        print("未找到环境变量 chinaTelecomAccount")
        return
    chinaTelecomAccount = acct_val
    for i in re.split(r'[\n&]+', chinaTelecomAccount.strip()):
        i = i.strip()
        if not i:
            continue
        if '#' in i:
        ...
        uid = parts[2].strip() if len(parts) > 2 else (parts[-1].strip() if len(parts) > 0 else "")
        ticket = False
